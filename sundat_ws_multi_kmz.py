...
api_key = "<APIKEY>"
workflow_name = "sundat_runner_v1_multi_kmz"
...
inputs = {
    "jsonin": {
        "src": "hda",
        "id": "sample_inputs/test.json"
    },
    "zipin": {
        "src": "hda",
        "id": "sample_inputs/test.zip"
    }
}
...
# get the HTML result dataset url
...
print("Final Result Written to out.html")
f = open('out.html','w')
f.write(req.text)
f.close()
...
#f = open('out.png','wb')
#f.write(c.download_jobfile(djid,"output/case_0_0/out.png"))
#f.close()

#print("Fetching example individual result file to out.skp...")
#f = open('out.skp','wb')
#f.write(c.download_jobfile(djid,"output/case_0_0/out.skp"))
#f.close()
